...
标的池不限死，支持**全市场检索**（约 7200 条）：

| 类型 | 来源 | 数量 |
|------|------|------|
| `stock` 个股 | `ak.stock_info_a_code_name()` | ~5500 |
| `etf` ETF | `ak.fund_etf_spot_em()` | ~1600 |
| `index` 指数 | `engine/indexes.py` 静态表（26 个主流宽基/红利/行业指数） | 26 |

代码格式统一为「6 位数字 + `.SH`/`.SZ`」（如 `600519.SH` / `000300.SH` / `510300.SH`），
指数与个股共用同一套取数、缓存与回测链路，靠 `engine/indexes.py` 的静态表区分类型。

前端标的输入框支持三种检索方式：

- **代码**：`600519` 或 `600519.SH`
- **名称**：`茅台`、`沪深300`
- **拼音首字母**：`gzmt` → 贵州茅台、`hs300` → 沪深300（需 `pypinyin`，已列入依赖）

`engine/symbols.py` 把全市场列表落盘为 JSON 快照（默认 7 天刷新），进程内内存检索，
应用启动时后台线程预热 —— 因此**搜索不会按每次键入去打网络**。
网络异常时回退到内置的热门标的表，保证搜索框永远可用。
快照与行情缓存都放在 `engine/cache/`，compose 里挂了 `quant_data` 命名卷持久化。

> 东财的指数列表接口（`stock_zh_index_spot_em`）在服务器 IP 上会被限流，
> 且用户实际关心的指数就是那二十来个主流品种，因此指数改用静态表 —— 更稳、更快、可读。

### 数据源与容灾

`engine/data.py` 按标的类型选择取数优先级，全部失败才回退合成数据，
...
> 实测东方财富的**指数**接口在服务器 IP 上长期被限流，故指数以腾讯为主源。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/catalog` | 策略 / 周期 / 默认参数 / 常用标的 / 标的表统计 |
| GET | `/api/symbols?q=茅台&type=stock,etf,index&limit=30` | 全市场标的搜索（代码 / 名称 / 拼音首字母） |
...
`/api/symbols` 的 `q` 留空时返回常用标的，`type` 留空表示全部类型。
